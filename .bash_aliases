...
file_name=$(basename "$file");
  ...
  then
    ...
    (cd "$file"&&zip -r -q - .) | curl --progress-bar --upload-file "-" "https://xfr.anonpasta.rocks/$file_name"|tee /dev/null;
    echo " << download zip file";
  else
    cat "$file"|curl --progress-bar --upload-file "-" "https://xfr.anonpasta.rocks/$file_name"|tee /dev/null;
    echo " << download file";
  fi;

else
file_name=$1;
curl --progress-bar --upload-file "-" "https://xfr.anonpasta.rocks/$file_name"|tee /dev/null;
echo " << download file";
fi;
}
